...
class MziTuSpider:
    ...
    def __init__(self, channel, page_num, process_num, sleep_time, timeout=7):
        ...
        if 1 > process_num or channel not in self.channels.keys():
    ...
    def handle_package(cls, queue, sleep_time, timeout):
        ...
        while True:
            ...
            # 图集包处理方法
                ...
                img_url = img_url[: img_url.rfind('0')]
                print('【开始下载图集】', name)
...
def main():
    ...
    mm_spider = MziTuSpider(channel, page_num, process_num, sleep_time)
    mm_spider.run()
